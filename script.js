...
recognition.onresult=(event)=>{
   ...
   setTimeout(()=>{
    content.innerText.style.opacity=0;
   },3000);

   setTimeout(()=>{
    content.textContent="Click here for talk to me";
    content.innerText.style.opacity=1;
   },2000);
...
btn.addEventListener("click",()=>{
    recognition.start();
    voice.style.display="block"; // it block is reverse of none
    btn.style.display="none"; // it invisible the button
})
...
function takeCommand(message){
   voice.style.display="none";
    ...
    if(message.includes("hello")||message.includes("hey")){
        ...
        speak("i am virtual assistant ,created by vishal");
    }
    else if (message.includes("youtube")) {
    
        const apiKey = "AIzaSyAga6BmVJuSp0mw1T9zOH3YDC4Ww9cTpkw"; //API key variable
...
setInterval(change_image, 3000); // it repeat the function image after the end of the image array after 3 sec of the interval
